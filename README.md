...
3. 执行 V1.2 正赛数据分析专项。
4. 等 V1.2 正赛报告稳定后，再进入 V2。
5. V2 完成历史复盘和事件解耦后，再进入 V3。

不要跳过 V1.2 直接做 V3。因为没有完整正赛数据分析能力，V3 的策略模拟和弯角级报告容易变成假分析。
...
```text
docs/v1-2-v2-v3-roadmap
```

提交信息：

```text
docs: add V1.2 V2 and V3 roadmap
```

本次验收标准：

- 项目规划文件中新增 V1.2 / V2 / V3 完整路线图。
- V1.2 明确预留正赛数据分析阶段。
- V2/V3 与 V1.2 边界清晰。
- 没有修改业务代码。
- 没有删除已有规划。
- Markdown 格式正确。
- 如果已有简略 V2/V3 内容，应合并为完整版，不重复堆叠。
