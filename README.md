...
当前开发进度：`2.0.0-beta4`，V2 比赛工程台基础功能已完成，第四版 beta 聚焦胎温 / 胎压实时接入和动态胎温 TTS 提醒。V3 基础能力已通过 PR #55 合并到 `main`，不改变发布版本号。
...
V3 基础能力已通过 PR #55 合并到 `main`：赛道分段、弯角指标、历史弯角分析页、stint 策略时间线、undercut / overcut 风险模型、压缩 AI 工程师报告和 SQLite 扩展表。本次不改变发布版本号，也不包含安装包发布。
